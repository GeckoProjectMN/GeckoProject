...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Geckocoin address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no geckocoin: URI
    if(!uri.isValid() || uri.scheme() != QString("geckocoin"))
        return false;
...
{
    // Convert geckocoin:// to geckocoin:
    //
    //    Cannot handle this later, because geckocoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("geckocoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "geckocoin:");
    }
...
{
    QString ret = QString("geckocoin:%1").arg(info.address);
    int paramCount = 0;
...
void openConfigfile()
{
    ...
    /* Open geckocoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Geckocoin Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Geckocoin Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Geckocoin Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "Geckocoin Core*.lnk"
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "geckocoincore.desktop";
    return GetAutostartDir() / strprintf("geckocoincore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a geckocoincore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Geckocoin Core\n";
        else
            optionFile << strprintf("Name=Geckocoin Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Geckocoin Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Geckocoin Core app to startup item list
