...
const char * const BITCOIN_CONF_FILENAME = "geckocoin.conf";
const char * const BITCOIN_PID_FILENAME = "geckocoind.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\GeckocoinCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\GeckocoinCore
    // Mac: ~/Library/Application Support/GeckocoinCore
    // Unix: ~/.geckocoincore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "GeckocoinCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/GeckocoinCore";
#else
    // Unix
    return pathRet / ".geckocoincore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty geckocoin.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override geckocoin.conf
