...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 5 GECKOCOIN irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on geckocoincentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by smartnode configured in geckocoin.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!dmn) {
        ...
        resultsObj.push_back(Pair("geckocoin.conf", statusObj));
...
#endif // ENABLE_WALLET
            "  vote-conf          - Vote on a governance object by smartnode configured in geckocoin.conf\n"
#ifdef ENABLE_WALLET
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Geckocoin features */
    { "geckocoin",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "geckocoin",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "geckocoin",               "gobject",                &gobject,                true,  {} },
    { "geckocoin",               "voteraw",                &voteraw,                true,  {} },
