...
# geckocoin_hash
RUN git clone https://github.com/geckocoin/geckocoin_hash
RUN cd geckocoin_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} geckocoin
RUN useradd -u ${USER_ID} -g geckocoin -s /bin/bash -m -d /geckocoin geckocoin
...
RUN mkdir /geckocoin-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /geckocoin-src && \
  chown $USER_ID:$GROUP_ID /cache && \
  chown $USER_ID:$GROUP_ID /cache -R
WORKDIR /geckocoin-src
